...
if scatter_df.empty:
    ...
    st.caption(
        ...
        "campione esteso a 599 comuni (2026-07-23) **% urbano è tornato "
        "non significativo** (p=0.370, coefficiente +0.0020 — era "
        "significativo, p=0.031, a n=234), mentre **NDVI è diventato "
        "significativo** (p=0.012) con un **coefficiente negativo** "
        "(-0.38, più vegetazione → temperatura più bassa) — il segno "
        "atteso fisicamente, a differenza delle versioni precedenti dove "
        "NDVI, quando significativo, aveva sempre un segno controintuitivo "
        "(più verde → più caldo) — solo l'elevazione resta un predittore "
        "robusto e stabile in ogni versione del modello. Il fatto che % "
        "urbano e NDVI continuino a scambiarsi di significatività (e "
        "talvolta di segno) a ogni allargamento del campione è un segnale "
        "che il modello non si è ancora stabilizzato: da trattare come "
        "risultato provvisorio, non come una conclusione solida sull'uso "
        "del suolo, finché non si osserva convergenza su più estensioni "
        "consecutive. Risultato ancora provvisorio (n=599 comuni)."
    )
